...
let UITransform = (_dec = (0, _index.ccclass)('cc.UITransform'), _dec2 = (0, _index.help)('i18n:cc.UITransform'), _dec3 = (0, _index.executionOrder)(110), _dec4 = (0, _index.menu)('UI/UITransform'), _dec5 = (0, _index.displayOrder)(0), _dec6 = (0, _index.tooltip)('i18n:ui_transform.content_size'), _dec7 = (0, _index.displayOrder)(1), _dec8 = (0, _index.tooltip)('i18n:ui_transform.anchor_point'), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = (0, _index.disallowMultiple)(_class = (0, _index.executeInEditMode)(_class = (_class2 = (_temp = _class3 = class UITransform extends _index2.Component {
  ...
  set contentSize(value) {
    ...
    if (_internal253Aconstants.EDITOR) {
      ...
      this.node.emit(_nodeEvent.NodeEventType.SIZE_CHANGED, clone);
    } else {
      this.node.emit(_nodeEvent.NodeEventType.SIZE_CHANGED);
    }
  ...
  set anchorPoint(value) {
    ...
    this.node.emit(_nodeEvent.NodeEventType.ANCHOR_CHANGED, this._anchorPoint);
  ...
  constructor() {
    ...
    if (_internal253Aconstants.UI_GPU_DRIVEN) {
      ...
      this._rectWithScale = new _index3.Vec3();
      this._anchorCache = new _index3.Vec3();
    }
  }
  ...
  onEnable() {
    this.node.on(_nodeEvent.NodeEventType.PARENT_CHANGED, this._parentChanged, this);

    this._markRenderDataDirty();
  }

  onDisable() {
    this.node.off(_nodeEvent.NodeEventType.PARENT_CHANGED, this._parentChanged, this);
  }
    ...
    for (let i = 0; i < cameras.length; i++) {
      ...
      if (testPt.x >= 0 && testPt.y >= 0 && testPt.x <= w && testPt.y <= h) {
        ...
        if (nodeEventProcessor && nodeEventProcessor.maskList) {
          ...
          const length = maskList ? maskList.length : 0; // find mask parent, should hit test it
  ...
  checkAndUpdateRect(rot, scale) {
    if (this._rectDirty) {
      ...
      _index3.Vec3.transformQuat(this._anchorCache, _vec3a.set(lenX, lenY, 0), rot);
    }
  }

  setRectDirty(transformBit) {
    if (transformBit & _nodeEnum.TransformBit.RS) {
      this._rectDirty = true;
    }
  }
