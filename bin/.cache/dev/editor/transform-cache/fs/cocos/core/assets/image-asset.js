...
function isImageBitmap(imageSource) {
  return !!(_globalExports.legacyCC.sys.hasFeature(_globalExports.legacyCC.sys.Feature.IMAGE_BITMAP) && imageSource instanceof ImageBitmap);
}
...
let ImageAsset = (_dec = (0, _index.ccclass)('cc.ImageAsset'), _dec(_class = (_class2 = (_temp = _class3 = class ImageAsset extends _asset.Asset {
  ...
  reset(data) {
    if (isImageBitmap(data)) {
      ...
      this._nativeData = data; // @ts-expect-error internal api usage
    ...
    for (const extensionID of extensionIDs) {
      ...
      if (index !== -1 && index < preferedExtensionIndex) {
        ...
        if (tmpExt === '.astc' && (!device || !device.hasFeature(_index2.Feature.FORMAT_ASTC))) {
          ...
          continue;
        } else if (tmpExt === '.webp' && !_globalExports.legacyCC.sys.hasFeature(_globalExports.legacyCC.sys.Feature.WEBP)) {
          continue;
        }
