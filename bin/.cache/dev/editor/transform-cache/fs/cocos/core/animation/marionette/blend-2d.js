...
exports.validateSimpleDirectionalSamples = validateSimpleDirectionalSamples;
exports.sampleFreeformCartesian = sampleFreeformCartesian;
exports.sampleFreeformDirectional = sampleFreeformDirectional;
exports.SimpleDirectionalIssueSameDirection = exports.blendSimpleDirectional = void 0;
...
const blendSimpleDirectional = (() => {
  const CACHE_NORMALIZED_SAMPLE = new _index.Vec2();
  const CACHE_BARYCENTRIC_SOLUTIONS = {
    wA: 0,
    wB: 0
  };
  return function blendSimpleDirectional(weights, samples, input) {
    (0, _asserts.assertIsTrue)(weights.length === samples.length);
    ...
    if (_index.Vec2.strictEquals(input, _index.Vec2.ZERO)) {
      const iCenter = samples.findIndex(sample => _index.Vec2.strictEquals(sample, _index.Vec2.ZERO));
    ...
    for (let iSample = 0; iSample < samples.length; ++iSample) {
      ...
      if (_index.Vec2.equals(sample, _index.Vec2.ZERO)) {
        iCenter = iSample;
        continue;
      }

      const sampleNormalized = _index.Vec2.normalize(CACHE_NORMALIZED_SAMPLE, sample);

      const cosAngle = _index.Vec2.dot(sampleNormalized, input);
...
exports.blendSimpleDirectional = blendSimpleDirectional;
...
function validateSimpleDirectionalSamples(samples) {
  ...
  samples.forEach((sample, iSample) => {
    ...
    for (let iCheckSample = 0; iCheckSample < nSamples; ++iCheckSample) {
      ...
      if (_index.Vec2.equals(sample, checkSample, 1e-5)) {
 ...
 */
class SimpleDirectionalIssueSameDirection {
  constructor(samples) {
...
exports.SimpleDirectionalIssueSameDirection = SimpleDirectionalIssueSameDirection;
...
function sampleFreeform(weights, samples, value, getGradientBandCoords) {
  weights.fill(0.0);
  const pIpInput = new _index.Vec2(0, 0);
  const pIJ = new _index.Vec2(0, 0);
  ...
  for (let iSample = 0; iSample < nSamples; ++iSample) {
    ...
    for (let jSample = 0; jSample < nSamples; ++jSample) {
      ...
      const t = 1 - _index.Vec2.dot(pIpInput, pIJ) / _index.Vec2.lengthSqr(pIJ);
...
function solveBarycentric(a, b, p, resolutions) {
  ...
  // wA = (P x B) / (A x B)
  // wB = (P x A) / (B x A)
  const det = _index.Vec2.cross(a, b);

  if (!det) {
    resolutions.wA = 0.0;
    resolutions.wB = 0.0;
  } else {
    resolutions.wA = _index.Vec2.cross(p, b) / det;
    resolutions.wB = _index.Vec2.cross(p, a) / -det;
  }
...
const getGradientBandCartesianCoords = (pI, pJ, input, pIpInput, pIpJ) => {
  _index.Vec2.subtract(pIpInput, input, pI);

  _index.Vec2.subtract(pIpJ, pJ, pI);
};

const getGradientBandPolarCoords = (() => {
  const axis = new _index.Vec3(0, 0, 0); // buffer for axis

  const tmpV3 = new _index.Vec3(0, 0, 0); // buffer for temp vec3

  const pQueriedProjected = new _index.Vec3(0, 0, 0); // buffer for pQueriedProjected

  const pi3 = new _index.Vec3(0, 0, 0); // buffer for pi3

  const pj3 = new _index.Vec3(0, 0, 0); // buffer for pj3

  const pQueried3 = new _index.Vec3(0, 0, 0); // buffer for pQueried3
  ...
  return (pI, pJ, input, pIpInput, pIpJ) => {
    ...
    _index.Vec3.set(pQueriedProjected, input.x, input.y, 0.0);

    if (_index.Vec2.equals(pI, _index.Vec2.ZERO)) {
      aIJ = _index.Vec2.angle(input, pJ);
      aIQ = 0.0;
      angleMultiplier = 1.0;
    } else if (_index.Vec2.equals(pJ, _index.Vec2.ZERO)) {
      aIJ = _index.Vec2.angle(input, pI);
      aIQ = aIJ;
      angleMultiplier = 1.0;
    } else {
      aIJ = _index.Vec2.angle(pI, pJ);

      if (aIJ <= 0.0) {
        aIQ = 0.0;
      } else if (_index.Vec2.equals(input, _index.Vec2.ZERO)) {
        aIQ = aIJ;
      } else {
        _index.Vec3.set(pi3, pI.x, pI.y, 0);

        _index.Vec3.set(pj3, pJ.x, pJ.y, 0);

        _index.Vec3.set(pQueried3, input.x, input.y, 0);

        _index.Vec3.cross(axis, pi3, pj3);

        _index.Vec3.projectOnPlane(pQueriedProjected, pQueried3, axis);

        aIQ = _index.Vec3.angle(pi3, pQueriedProjected);

        if (aIJ < Math.PI * 0.99) {
          if (_index.Vec3.dot(_index.Vec3.cross(tmpV3, pi3, pQueriedProjected), axis) < 0) {
            aIQ = -aIQ;
          }
        }
      }
    ...
    const lenPI = _index.Vec2.len(pI);

    const lenPJ = _index.Vec2.len(pJ);

    const deno = (lenPJ + lenPI) / 2;

    _index.Vec2.set(pIpJ, (lenPJ - lenPI) / deno, aIJ * angleMultiplier);

    _index.Vec2.set(pIpInput, (_index.Vec3.len(pQueriedProjected) - lenPI) / deno, aIQ * angleMultiplier);
  };
})();