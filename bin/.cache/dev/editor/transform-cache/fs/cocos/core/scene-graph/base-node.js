...
let BaseNode = (_dec = (0, _index.ccclass)('cc.BaseNode'), _dec(_class = (_class2 = (_temp = _class3 = class BaseNode extends _object.CCObject {
  ...
  setParent(value, keepWorldTransform = false) {
    ...
    if (this.emit) {
      this.emit(_nodeEvent.NodeEventType.PARENT_CHANGED, oldParent);
    }
    ...
    if (oldParent) {
      if (!(oldParent._objFlags & Destroying)) {
        ...
        if (oldParent.emit) {
          oldParent.emit(_nodeEvent.NodeEventType.CHILD_REMOVED, this);
        }
      }
    }
    ...
    if (newParent) {
      ...
      if (newParent.emit) {
        newParent.emit(_nodeEvent.NodeEventType.CHILD_ADDED, this);
      }
    }
  ...
  on(type, callback, target, useCapture = false) {
    switch (type) {
      case _nodeEvent.NodeEventType.TRANSFORM_CHANGED:
  ...
  targetOff(target) {
    ...
    if (this._eventMask & TRANSFORM_ON && !this._eventProcessor.hasEventListener(_nodeEvent.NodeEventType.TRANSFORM_CHANGED)) {
      this._eventMask &= ~TRANSFORM_ON;
    }
  }
  ...
  _updateSiblingIndex() {
    ...
    this.emit(_nodeEvent.NodeEventType.SIBLING_ORDER_CHANGED);
  }
  ...
  _onPreDestroyBase() {
    ...
    if (!destroyByParent) {
      // remove from parent
      if (parent) {
        this.emit(_nodeEvent.NodeEventType.PARENT_CHANGED, this); // During destroy process, sibling index is not reliable
        ...
        if (parent.emit) {
          parent.emit(_nodeEvent.NodeEventType.CHILD_REMOVED, this);
        }
      }
    } // emit node destroy event (this should before event processor destroy)


    this.emit(_nodeEvent.NodeEventType.NODE_DESTROYED, this); // Destroy node event processor
