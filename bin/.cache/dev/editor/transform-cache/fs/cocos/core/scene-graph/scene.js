...
let Scene = (_dec = (0, _index.ccclass)('cc.Scene'), _dec(_class = (_class2 = (_temp = class Scene extends _baseNode.BaseNode {
  ...
  _init() {
    if (_internal253Aconstants.JSB) {
      this._nativeObj = new _nativeScene.NativeScene();
    }
  }
  ...
  constructor(name) {
    ...
    this._dirtyFlags = 0;
    this._lpos = _index2.Vec3.ZERO;
    this._lrot = _index2.Quat.IDENTITY;
    this._lscale = _index2.Vec3.ONE;
  ...
  destroy() {
    ...
    if (this._renderScene) _globalExports.legacyCC.director.root.destroyScene(this._renderScene);
    this._active = false;
    this._activeInHierarchy = false;
    return success;
  }
  ...
  _load() {
    if (!this._inited) {
      if (_internal253Aconstants.TEST) {
        (0, _debug.assert)(!this._activeInHierarchy, 'Should deactivate ActionManager by default');
      }

      (0, _utils.expandNestedPrefabInstanceNode)(this);
  ...
  _activate(active) {
    ...
    _globalExports.legacyCC.director._nodeActivator.activateNode(this, active); // The test environment does not currently support the renderer
