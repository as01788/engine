...
// @skipLibCheck
let CCSlot = (_dec = (0, _classDecorator.ccclass)('dragonBones.CCSlot'), _dec(_class = (_temp = class CCSlot extends _dragonbonesJs.Slot {
  static toString() {
  ...
  constructor() {
    ...
    this._localVertices = [];
    this._indices = [];
    this._matrix = new _index.Mat4();
    this._worldMatrix = new _index.Mat4();
    this._worldMatrixDirty = true;
    this._visible = false;
    this._color = new _index.Color();
  ...
  calculWorldMatrix() {
    ...
    if (parent) {
      ...
      _index.Mat4.copy(this._worldMatrix, this._matrix);
    }
  ...
  _onClear() {
    ...
    _index.Mat4.identity(this._matrix);

    _index.Mat4.identity(this._worldMatrix);
  ...
  _onUpdateDisplay() {} // just for adapt to dragonbones api,no need to do any thing
  ...
  _updateGlueMesh() {} // just for adapt to dragonbones api,no need to do any thing
  ...
  _updateFrame() {
    ...
    if (currentVerticesData) {
      ...
      const vertexCount = intArray[currentVerticesData.offset + _dragonbonesJs.BinaryOffset.MeshVertexCount];
      const triangleCount = intArray[currentVerticesData.offset + _dragonbonesJs.BinaryOffset.MeshTriangleCount];
      let vertexOffset = intArray[currentVerticesData.offset + _dragonbonesJs.BinaryOffset.MeshFloatOffset];
      ...
      for (let i = 0; i < triangleCount * 3; ++i) {
        indices[indexOffset++] = intArray[currentVerticesData.offset + _dragonbonesJs.BinaryOffset.MeshVertexIndices + i];
      }
  ...
  _updateMesh() {
    ...
    if (weightData) {
      ...
      const intArray = data.intArray;
      const floatArray = data.floatArray;
      const vertexCount = intArray[verticesData.offset + _dragonbonesJs.BinaryOffset.MeshVertexCount];
      let weightFloatOffset = intArray[weightData.offset + _dragonbonesJs.BinaryOffset.WeigthFloatOffset];

      if (weightFloatOffset < 0) {
        weightFloatOffset += 65536; // Fixed out of bouds bug
      }

      for (let i = 0, iB = weightData.offset + _dragonbonesJs.BinaryOffset.WeigthBoneIndices + bones.length, iV = weightFloatOffset, iF = 0, lvi = 0; i < vertexCount; i++, lvi += 4) {
      ...
      const isSurface = this._parent._boneData.type !== _dragonbonesJs.BoneType.Bone;
      const data = verticesData.data;
      const intArray = data.intArray;
      const floatArray = data.floatArray;
      const vertexCount = intArray[verticesData.offset + _dragonbonesJs.BinaryOffset.MeshVertexCount];
      let vertexOffset = intArray[verticesData.offset + _dragonbonesJs.BinaryOffset.MeshFloatOffset];
