...
let SubContextView = (_dec = (0, _index.ccclass)('cc.SubContextView'), _dec2 = (0, _index.help)('i18n:cc.SubContextView'), _dec3 = (0, _index.executionOrder)(110), _dec4 = (0, _index.requireComponent)(_uiTransform.UITransform), _dec5 = (0, _index.menu)('Miscellaneous/SubContextView'), _dec6 = (0, _index.tooltip)('i18n:subContextView.design_size'), _dec7 = (0, _index.tooltip)('i18n:subContextView.fps'), _dec(_class = _dec2(_class = _dec3(_class = _dec4(_class = _dec5(_class = (_class2 = (_temp = class SubContextView extends _component.Component {
  ...
  constructor() {
    ...
    this._texture = new _index5.Texture2D();
  }

  onLoad() {
    if (_minigame.minigame.getOpenDataContext) {
      this._updateInterval = 1000 / this._fps;
      this._openDataContext = _minigame.minigame.getOpenDataContext();
  ...
  _updateSubContextView() {
    ...
    const viewportRect = _view.view.getViewportRect();

    const box = contentTrans.getBoundingBoxToWorld();

    const visibleSize = _view.view.getVisibleSize();

    const dpr = _screenAdapter.screenAdapter.devicePixelRatio; // TODO: the visibleSize need to be the size of Canvas node where the content node is.
  ...
  _registerNodeEvent() {
    this.node.on(_nodeEvent.NodeEventType.TRANSFORM_CHANGED, this._updateSubContextView, this);
    this.node.on(_nodeEvent.NodeEventType.SIZE_CHANGED, this._updateSubContextView, this);
    this.node.on(_nodeEvent.NodeEventType.LAYER_CHANGED, this._updateContentLayer, this);
  }

  _unregisterNodeEvent() {
    this.node.off(_nodeEvent.NodeEventType.TRANSFORM_CHANGED, this._updateSubContextView, this);
    this.node.off(_nodeEvent.NodeEventType.SIZE_CHANGED, this._updateSubContextView, this);
    this.node.off(_nodeEvent.NodeEventType.LAYER_CHANGED, this._updateContentLayer, this);
  }
