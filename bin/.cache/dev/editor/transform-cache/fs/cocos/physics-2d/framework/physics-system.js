...
class PhysicsSystem2D extends (0, _index.Eventify)(_index.System) {
  ...
  static get PhysicsGroup() {
    return _physicsTypes.PhysicsGroup;
  }
  /**
   * @en
    ...
    if (config) {
      ...
      if (config.collisionGroups) {
        ...
        if (cg instanceof Array) {
          cg.forEach(v => {
            _physicsTypes.PhysicsGroup[v.name] = 1 << v.index;
          });

          _index.Enum.update(_physicsTypes.PhysicsGroup);
        }
      }
    }
...
function initPhysicsSystem() {
  if (!PhysicsSystem2D.PHYSICS_NONE && !_internal253Aconstants.EDITOR) {
    _index.director.registerSystem(PhysicsSystem2D.ID, PhysicsSystem2D.instance, _index.System.Priority.LOW);
  }
}