    ...
    for (let i = 0; i < this._struct.primitives.length; i++) {
      ...
      if (prim.vertexBundelIndices.length > 0) {
        ...
        for (let j = 0; j < attrs.length; ++j) {
          const attr = attrs[j];
          gfxAttributes[j] = new _index4.Attribute(attr.name, attr.format, attr.isNormalized, attr.stream, attr.isInstanced, attr.location);
        }
      }
...
}), _descriptor2 = _applyDecoratedDescriptor(_class2.prototype, "_hash", [_index.serializable], {
  configurable: true,
  enumerable: true,
  writable: true,
  initializer: function () {
